...
const validatePin = (pin) => /^\d\d\d\d$/g.test(pin);
/* ------------------------------------------------------------------------------------------------
CHALLENGE 3
...
// email validation regex found on emailregex.com that I tried and worked around
...
// const validatePhoneNumber = (phoneNumber) => /\(*\d\d\d\)*[ -]*\d\d\d[ -]*\d\d\d\d$/.test(phoneNumber);
...
const findTagNames = elements => elements.reduce((acc, str) => [...acc, ...str.match(/\/\w+/g)] , [] );
