...
const splitFoods = (recipe) => {
  ...
  for (let ingredient of recipe.ingredients) {
    ...
    let ingredientType = ingredientArray.splice(2);
    result.push(ingredientType.join(' '));
  }
  return result;
};
...
Write a function named stepAction tipe and extracts the action verbs from the steps. Fortunate for you, the action verbs are the first word of each action.hat takes in the rec
...
const totalSumCSV = (str) => {
  ...
  return numsArray.reduce((total, currValue) => total + parseInt(currValue), 0);
};
...
const removeVowels = (str) => {
  ...
  }for (let letter of tempArray) {
    strArray.splice(strArray.indexOf(letter), 1);
...
const extractVowels = (str) => {
  ...
  const tempArraySorted = tempArray.sort((a, b) => {
    if (a>b) {
      return 1;
    } return -1;
  });
  return [strArray.join(''), tempArraySorted.join('')];
};
