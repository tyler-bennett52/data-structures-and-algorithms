...
// const addValues = (arr) => arr.reduce((accumulator, currValue) => accumulator + currValue, 0);
const addValues = (arr) => arr.reduce((total, x) => total + x, 0);
...
const addPurchases = (arr) => arr.reduce((total, x) => total + x.purchasePrice, 0);
...
const countNumberOfElements = (arr) => arr.reduce((accumulator, currValue) => accumulator + 1, 0);
/* ------------------------------------------------------------------------------------------------
CHALLENGE 5
...
const returnNames = (arr) => arr.reduce((accumulator, currValue) => [...accumulator, currValue.name], []);
/* ------------------------------------------------------------------------------------------------
CHALLENGE 6
...
const reversedString = (str) => {
  ...
  return strArray.reduce((accumulator, currValue) => currValue + accumulator, '');
};
/* ------------------------------------------------------------------------------------------------
...
const countNumberOfChildren = (arr) => arr.reduce((total, currValue) => {
  if (currValue.children === undefined) {
    return total;
  } else {
    return total + currValue.children.length;
  }
}, 0);
/* ------------------------------------------------------------------------------------------------
...
const calculateAverage = (arr) => arr.reduce((total, currValue, index, array) => total + (currValue/array.length) , 0);
/* ------------------------------------------------------------------------------------------------
...
const countPrimeNumbers = (arr) => arr.reduce((total, currValue) =>{
  if (isPrime(currValue) === true) {
    return total + 1;
  } return total;
}, 0);
...
const extractStats = (snorlaxData) => {
  const allMoves = snorlaxData.stats.reduce((total, currValue) => {
    total[currValue.stat.name] = currValue.baseStat;
    return total;
  },
  {});
  return allMoves;
...
const extractChildren = (arr) => {
  const allAs = arr.filter(person => person.name.includes('a'));
  return allAs.reduce((total, currValue) => {
    if (currValue.children === undefined) {
      return [...total];
    } return [...total, ...currValue.children];
  } , []);
};
