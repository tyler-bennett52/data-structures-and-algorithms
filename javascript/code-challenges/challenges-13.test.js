...
const findEvery = (arr, target) => arr.length === findAnything(arr, target).length;
...
const unenrollBrook = (arr) => arr.reduce((acc, val) => [...acc, val.filter(str => !str.includes('Brook'))] ,[]);
...
const sortByDay = (arr) => {
  const bigArray = [];
  for (let day of daysOfWeek) {bigArray.push([]);}
  for (let event of arr) {
    for (let i = 0; i < daysOfWeek.length; i++) {
      if (event.includes(daysOfWeek[i])) {
        bigArray[i].push(event);
      }
    }
  } return bigArray;
};
