...
E.g. [4,2,7,5,9,2] -> 9
------------------------------------------------------------------------------------------------ */
const maxInArray = (arr) => arr.reduce((total, currValue) => currValue > total ? currValue : total);
...
const totalCharacters = (arr) => arr.reduce((acc, val) => {
  acc += 1;
  val.spouse ? acc += 1 : null;
  val.children ? acc += val.children.length : null;
  return acc;
}, 0);
...
const houseSize = (arr) => {
  ...
  for (let character of arr) {
    let numMembers = 1;
    character.spouse ? numMembers += 1 : null;
    character.children ? numMembers += character.children.length : null;
    sizes.push({house: character.house, members: numMembers});
  }
  return sizes;
};
...
const houseSurvivors = (arr) => {
  ...
  for (let character of arr) {
    let numMembers = 1;
    character.spouse ? numMembers++ : null;
    deceasedSpouses.includes(character.spouse) ? numMembers-- : null;
    character.children ? numMembers += character.children.length : null;
    survivors.push({house: character.house, members: numMembers});
  }
  return survivors;
};
