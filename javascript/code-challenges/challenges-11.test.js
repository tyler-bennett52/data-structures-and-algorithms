...
const totalSum = (input) => {
  let total = 0;
  for (let arr of input) {
    total += arr.reduce((accumulator, currValue) => accumulator += currValue);
  } return total;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  for (let arr of input) {
    ...
    const arrFilter = arr.filter(x => x % 5 === 0);
    const arrRaised = arrFilter.map(x => 2 ** x);
    bigTempArray.push(arrRaised);
  }
  return bigTempArray;
};
...
let findShortest = (data) => data.sort((a,b) => a.height - b.height)[0].name;
